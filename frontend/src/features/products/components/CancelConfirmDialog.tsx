...
export function CancelConfirmDialog({ open, onKeepGoing, onDiscard, title, body, keepGoingLabel, discardLabel }: CancelConfirmDialogProps) {
      ...
      <AlertDialog.Portal>
        ...
        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-white/10 bg-slate-900 p-6 shadow-xl shadow-slate-950/40">
          <AlertDialog.Title className="text-lg font-semibold text-slate-100">
            {title ?? t('products.addProduct.cancelConfirmTitle')}
          </AlertDialog.Title>
          <AlertDialog.Description className="mt-2 text-sm text-slate-400">
            {body ?? t('products.addProduct.cancelConfirmBody')}
          ...
          <div className="mt-6 flex justify-end gap-3">
            <AlertDialog.Cancel asChild>
              ...
              >
                {keepGoingLabel ?? t('products.addProduct.cancelConfirmKeepGoing')}
              </button>
            </AlertDialog.Cancel>
            <AlertDialog.Action asChild>
              ...
              >
                {discardLabel ?? t('products.addProduct.cancelConfirmDiscard')}
              </button>
            </AlertDialog.Action>
          </div>
        </AlertDialog.Content>
