...
export const elCorteInglesVerifier: VerifierFn = async ({ context, productId, url }) => {
  try {
    logger.info(
      { event: VERIFIER_LOG_EVENT.VERIFICATION_STARTED, provider: 'elCorteIngles', productId },
      'Starting El Corte Ingles verification',
    );
const page = await context.newPage();
    ...
    logger.info(
      { event: VERIFIER_LOG_EVENT.VERIFICATION_IMAGE_OBTAINED, provider: 'elCorteIngles', productId },
      'El Corte Ingles product image obtained',
    );
    ...
    logger.error(
      {
        ...
        provider: 'elCorteIngles',
        productId,
        error: normalizeLogError(error),
      },
      'El Corte Ingles verification failed',
    );
    throw error;
  }
};
